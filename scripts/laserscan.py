...
# Script edited for custom loading
...
class LaserScan:
  ...
  def reset(self):
    ...
    self.proj_xyz = np.full((self.proj_H, self.proj_W, 3), self.no_data_val,
                            dtype=np.float32)

    # projected remission - [H,W] intensity (-1 is no data)
    self.proj_remission = np.full((self.proj_H, self.proj_W), self.no_data_val,
                                  dtype=np.float32)

    # projected index (for each pixel, what I am in the pointcloud)
    # [H,W] index (-1 is no data)
    self.proj_idx = np.full((self.proj_H, self.proj_W), self.no_data_val,
                            dtype=np.int32)
  ...
  # Modified to suit purpose
  def open_scan(self, points_xyz, points_intensity):
    """ Open raw scan and fill in attributes
    """
    # reset just in case there was an open structure
    self.reset()
  ...
  #   remissions =pc.pc_data['intensity']
    ...
    # put in attribute
    self.set_points(points_xyz, points_intensity)
  ...
  def do_range_projection(self):
    ...
    if threshold_by_range:
      ...
      print("[scan2shape/laserscan:] thresholding range and only training points within ", range_threshold, " meters!")
    else:
      # get scan components
      scan_x = self.points[:, 0]
      scan_y = self.points[:, 1]
